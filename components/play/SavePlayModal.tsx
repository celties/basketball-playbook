  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
      <div className="bg-gray-900 border border-gray-700 rounded-xl p-6 w-[400px] shadow-2xl">
        <h2 className="text-white font-bold text-lg mb-4">プレイを保存</h2>

        <div className="mb-4">
          <label className="text-gray-400 text-xs block mb-1">プレイ名 (コール)</label>
          <input
            type="text"
            value={name}
            ...
            autoFocus
            className="w-full bg-gray-800 text-white px-3 py-2 rounded border border-gray-600 focus:outline-none focus:border-blue-400 placeholder-gray-500"
          />
        </div>

        <div className="mb-4">
          <label className="text-gray-400 text-xs block mb-2">カテゴリ</label>
        ...
        <div className="mb-6">
          <label className="text-gray-400 text-xs block mb-1">メモ（任意）</label>
          <textarea
            value={description}
            onChange={(e) => setDescription(e.target.value)}
            placeholder="プレイの説明・ポイントなど..."
            rows={3}
            className="w-full bg-gray-800 text-white px-3 py-2 rounded border border-gray-600 focus:outline-none focus:border-blue-400 placeholder-gray-500 resize-none text-sm"
          />
        </div>
        ...
        <div className="flex gap-3">
          <button
            onClick={onClose}
            className="flex-1 py-2 rounded border border-gray-600 text-gray-300 hover:bg-gray-800 transition-colors"
          >
            キャンセル
          </button>
          <button
